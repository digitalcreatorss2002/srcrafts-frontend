...
export async function getProducts(slug, filters = {}, page = 1) {
    ...
    const res = await apiFetcher(`/api/products/categories/${slug}?${query}`);
  
    // Edge Case 1 & 2: Agar products nahi mile
    if (data.products.length === 0) {
      // Search similar products in the same category or general search
      const fallbackRes = await apiFetcher(`${process.env.NEXT_PUBLIC_API_URL}/api/products/search?limit=10`);
...
export async function getProductsCached(slug, filters, page = 1) {
  'use cache'; // Opt-in to Next.js 16 caching - Invalided to force fetch new recursive categories
  ...
  const url = `/api/products/categories/${slug}?${queryParams}`;
  ...
  try {
    ...
    console.error("Fetch Error in getProductsCached:");
    console.log(error);
